 ...
 * $Id: dynamicBuild.c,v 1.9 2002/09/13 05:14:31 katayama Exp $
 */
...
dynamicBuild(const char *recordFileName, int maxCount,
	     const char *indexFileName, int dataItemSize,
	     const HnPropertiesSt *properties, HnBool debug, HnBool onMemory)
{
    ...
    indexFile = HnSRTreeFileSt_create((onMemory ? NULL : indexFileName),
    ...
    if ( onMemory ) {
	if ( !HnSRTreeFileSt_dumpToFile(indexFile, indexFileName) ) {
