...
*/
double dml::ComputeObjectAngle(sg::Vector t, sg::Vector v)
{
...
/*!
	@brief
*/
void dml::GetBoundaryInfoAtJoint(const SkelJoint* pJoint, sg::BoundaryInfoList *pBil,
								 SkelBranch* pExcludeBranch /* = NULL */)
{
	...
	forall_const_branches(pBranch, pJoint->edges)
	{
		...
		{
			idx = (pBranch->getFirstXYPoint() == pJoint->fp.p)
				? 0 : pBranch->size() - 1;

			ASSERT(pBranch->getFluxPoint(idx).p == pJoint->fp.p);

			pBil->push_back(pBranch->getBoundaryInfo(idx));
		}
	}
}

//! Find the closest boundary points between the branches
void dml::GetCommonBndryPoint(const SkelBranch* b0, const SkelBranch* b1,
...
*/
double dml::DiskSectorArea(const sg::FluxPoint& fp, const sg::BoundaryInfo& bi)
{
...
*/
inline double dml::IntraSpokesArea(const SkelPtCoord& p0, const SkelPtCoord& b0,
							  const SkelPtCoord& p1, const SkelPtCoord& b1)
{
	...
	double upperArea = TriangleArea(p0b0, b0b1, p0b1);
	double lowerArea = TriangleArea(p0b1, p1b1, p0p1);
	...
	if (b0b1 > 3 * p0p1)
	{
		...
		else
		{
			...
			area2 = TriangleArea(p1b0, p0b0, p0p1);
		}
...
*/
double dml::BranchSegmentArea(const sg::FluxPointList& fpl,
							  const sg::BoundaryInfoList& bil,
...
/*!
*/
void dml::ComputeTangentsAtJoint(SkelJoint* pJoint, const SkelEdges& branches,
...
*/
void dml::ComputeTangentAnglesAtJoint(SkelJoint* pJoint, const SkelEdges& branches,
...
*/
double dml::ComputeObjectAnglesAtJoint(SkelJoint* pJoint, const SkelEdges& branches,
...
*/
void dml::ComputeAreasAtJoint(SkelJoint* pJoint, const SkelEdges& branches,
...
*/
void dml::ComputeObjectAngleVariance(const SkelEdges& branches, BranchInfoVect& variances)
{
...
*/
bool dml::DoSpokesIntersect(const sg::Point& S1_P0, const sg::Point& S1_P1,
							const sg::Point& S2_P0, const sg::Point& S2_P1)
{
	if (S1_P1 == S2_P1)
		return false; // spokes have the same endpoint. not an intersection.

	dml::VECTOR u(S1_P1.x - S1_P0.x, S1_P1.y - S1_P0.y);
    dml::VECTOR v(S2_P1.x - S2_P0.x, S2_P1.y - S2_P0.y);
    dml::VECTOR w(S1_P0.x - S2_P0.x, S1_P0.y - S2_P0.y);
    ...
    if (fabs(D) < SMALL_NUM)            // S1 and S2 are parallel
	{
		...
		dml::VECTOR w2(S1_P1.x - S2_P0.x, S1_P1.y - S2_P0.y);
...
*/
double dml::ComputeSpokeAngleAtJoint(const SkelBranch* pBranch, SkelJoint* pJoint)
{
	SkelPtIndex i = GetEndpointIndex(pBranch, pJoint);

	const SkelPt& fp = pBranch->getFluxPoint(i);
	const SkelPtBndryInfo& bi = pBranch->getBoundaryInfo(i);
...
*/
bool dml::DoBranchSpokeIntersect(const SkelBranch* pBranch,
...
*/
unsigned int dml::GetUniqueInnerBoundaryPoint(const SkelBranch* pBranch, char side)
{
	const sg::BoundaryInfoList& bil = pBranch->getBoundaryInfoList();
