...
class CmdLineException; //Forward Declaration of the class contained in CmdLineException.h
...
public:
	...
	*/
	int SplitLine(int argc, char **argv) throw(CmdLineException);
	...
	*/
	bool HasSwitch(const char *pSwitch) throw(CmdLineException);
	...
	*/
	string  GetSafeArgument(const char *pSwitch, unsigned int iIdx, const char *pDefault) throw(CmdLineException);
	...
	*/
	int GetArgumentCount(const char *pSwitch) throw(CmdLineException);

	/*!
	*	\brief Print Help on screen, Throw an exception if the file is not found.
	*	\param filepath : Path of the help file.
	*/
	void ShowHelp(const char* filepath = "help.txt") throw(CmdLineException);
