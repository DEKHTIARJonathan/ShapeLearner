...
/*!
	...
	in a funny way.
*/
void BoundaryPointFinder::FixSpecialCrossingCase(SkelBranch* pBranch, char side)
{
...
*/
sg::BoundaryPoint BoundaryPointFinder::FindClosestBoundaryPoint(
	const sg::Point& p0, const sg::Point& p1, const BoundaryInterval& bndryInt) const
{
	...
	{
		m_origBndryPts.GetDataPoint(i, pt.x, pt.y);
...
*/
void BoundaryPointFinder::FixCrossingEndpointSpokes(SkelBranch* pBranch, char side)
{
	const sg::FluxPointList& fpl = pBranch->getFluxPoints();
	sg::BoundaryInfoList& bil    = pBranch->getBoundaryInfoList();
	...
	if (!pBranch->isTerminal())
	{
		...
		BoundaryInterval bndryInt(m_origBndryPts.Size(), firstIdx, lastIdx);

		char otherSide = GetOtherSide(side);
...
bool GetRobustEndpointForwardTangent(SkelBranch* pBranch, SkelPtIndex idx, sg::Vector* pTan)
{
	...
	const sg::FluxPoint& fp0     = pBranch->getFluxPoint(idx0);
	const sg::BoundaryPoint& bp0 = pBranch->getBoundaryInfo(idx0).first;
	...
	{
		bool bDec = fp0.radius() < pBranch->getFluxPoint(idx).radius();
...
int BoundaryPointFinder::GetValidBoundaryIndex(const SkelBranch* pBranch,
											   char side, const sg::BoundaryPoint& bp)
{
	...
	{
		const sg::BoundarySegmentLists& bsl = pBranch->getBoundarySegments();

		sg::Point pt = bsl[side][bp.subindex][-bp.index - 1];
		double epsilon = 0;

		int n = m_origBndryPts.RangeSearch(pt.x, pt.y, 2, &epsilon, 2, 1);
		ASSERT(n > 0);

		return m_origBndryPts.GetNNIndex(0);
	}
}
...
*/
bool BoundaryPointFinder::TestBoundaryInterval(const int firstIdx, const int lastIdx,
...
void BoundaryPointFinder::SetBoundaryInterval(const int firstIdx, const int lastIdx,
											  const int otherIdx0, const int otherIdxN,
											  BoundaryInterval* pBndryInt) const
{
	...
	pBndryInt->Set(m_origBndryPts.Size(), firstIdx, lastIdx);
...
void BoundaryPointFinder::SetBoundaryInterval(const char side,
											  const sg::BoundaryInfo& bi0,
											  const sg::BoundaryInfo& biN,
											  BoundaryInterval* pBndryInt) const
{
	const char otherSide = GetOtherSide(side);
...
void BoundaryPointFinder::SetBoundaryInterval(const SkelBranch* pBranch, const char side,
...
void BoundaryPointFinder::EnsureValidInterval(const sg::BoundaryInfo& bi0, sg::BoundaryInfo* pBiN)
{
	...
	BoundaryInterval bndryInt(m_origBndryPts.Size(), bi0.first.index, pBiN->first.index);
...
#ifdef _DEBUG
			bndryInt.Set(m_origBndryPts.Size(), bi0.first.index, pBiN->first.index);
...
*/
void BoundaryPointFinder::AssignBoundaryPoints(sg::DDSGraph* pDDSGraph)
{
	JointAngles angles;
	sg::Vector tangent;
	SkelBranch* pBranch;
...
*/
void BoundaryPointFinder::AssignInnerBoundaryPoints(const sg::FluxPointList& fpl, sg::BoundaryInfoList& bil)
{
	if (!m_bAreKDTreesBuilt)
		BuildKDTrees();
...
*/
void BoundaryPointFinder::AssignRadiusValues(const sg::BoundaryInfoList& bil, sg::FluxPointList& fpl)
{
...
*/
/*static*/ double BoundaryPointFinder::ComputeEndPointOverlap(const sg::FluxPointList& fpl)
{
...
*/
void BoundaryPointFinder::SetBoundaryInfoAtJoint(SkelJoint* pJoint, const JointAngles& angles)
{
	...
	// The unsmoothed points may be a better choice?
		...
		for (; i < n && anglesFound < angles.size(); i++)
		{
			...
			if (angle < angles.front().Angle() || angle > angles.back().Angle())
			{
				...
				if (fabs(alpha0) > 0 && fabs(alpha1) > 0)
				{
					...
					{
						bp0.set(annPt[0], annPt[1], bndryPts.GetNNIndex(i));
						bp1 = bp0;
						anglesFound++;
					}
				}
	...
	} while (anglesFound < angles.size() && n < bndryPts.Size());
	...
	do {
		...
		for (; i < n; i++)
		{
			...
			if (alpha >= 0 && bi0.first.index == -1)
			{
				...
				{
					if (bi0.first.index = pBranch->getFirstBndryInfo().first.index)
						bi0.first = pBranch->getLastBndryInfo().first;
					else
						bi0.first = pBranch->getFirstBndryInfo().first;
			...
			else if (alpha < 0 && bi0.second.index == -1)
			{
				...
				{
					if (bi0.second.index = pBranch->getFirstBndryInfo().second.index)
						bi0.second = pBranch->getLastBndryInfo().second;
					else
						bi0.second = pBranch->getFirstBndryInfo().second;
...
*/
void BoundaryPointFinder::SetInnerBoundaryInfo(const sg::FluxPointList& fpl,
											   ...
											   sg::BoundaryInfoList& bil)
{
	...
	// Use the smooth points to find the pair of internal bounadry points
	KDTree& bndryPts = m_smoothBndryPts;
	...
	for (k = 1; k < nLast; k++)
	{
		...
		} while ((bLookingForSide1 || bLookingForSide2) && n < bndryPts.Size());

		ASSERT(!bLookingForSide1 && !bLookingForSide2);

		// Done. Just copy the points' coordinates
		bndryPts.GetDataPoint(bi.first.index, bi.first.pt.x, bi.first.pt.y);
		bndryPts.GetDataPoint(bi.second.index, bi.second.pt.x, bi.second.pt.y);
	}
}
...
*/
double BoundaryPointFinder::BoundaryDistance(int nFrom, int nTo) const
{
	double dist = 0;
	int i;

	//const KDTree& bndryPts = m_origBndryPts;
	const KDTree& bndryPts = m_smoothBndryPts;

	if (nTo < nFrom) // must wrap around
	{
		for (i = 0; i < nTo; i++)
			dist += bndryPts.DataPointDistance(i, i + 1);
...
*/
void BoundaryPointFinder::AssignBoundaryDistanceInfo(const sg::FluxPointList& fpl,
...
*/
void BoundaryPointFinder::AssignAxisDistanceInfo(const sg::FluxPointList& fpl,
                                               sg::BoundaryInfoList& bil)
{
...
#ifdef USE_EXTRA_BOUNDARY_INFO
		bil[i].first.bar /= dist;  // this make the value in side 1's bar a real ratio
		bil[i].second.bar /= dist; // this make the value in side 2's bar a real ratio
#endif
	}
}