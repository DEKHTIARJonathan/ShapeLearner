...
public:
	...
	String(int n, int nBuffSize)
	{
		...
		if (szBuff)
		{
			sprintf_s(szBuff, sizeof(char)*nBuffSize, "%d", n);
	...
	{
		char szBuff[100];
		sprintf_s(szBuff, sizeof(szBuff), "%d", n);
	...
	String& operator=(const double& n)
	{
		char szBuff[100];
		//sprintf(szBuff, "%.3f", n);
		sprintf_s(szBuff, sizeof(szBuff), "%.3f", n);
		return operator=((const char*) szBuff);
	}
	...
	String& SetEqual(const char* sz, const int num)
	{
		...
		sprintf_s(szBuff, sizeof(char)*buffSize, "%s%d", sz, num);
